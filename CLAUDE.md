...
# Workflow mode (JSON-driven sequence)
python main.py --algo workflow --workflow-file workflow.example.json --workflow-name workflow_sample_1
```
...
utils/
  ...
  stats.py       ← Statistics class: RTT tracking per message id
stratums/
  stratum.py             ← Base class: send() helper, on_connect/on_disconnect hooks, self.stats
  ...
  stratum_workflow.py    ← JSON-driven workflow: replays a step sequence from a config file
```
...
**workflow exception:** server sends step_1 on connect (`on_connect` hook), then advances on each client response that contains an `id` field.

## Adding a new algorithm

1. Add a constant to `ALGORITHM` in `utils/algorithm.py` and return `True` for it in `is_valid_algorithm()`.
...
4. Add the branch in `Pool.__init__()` in `core/pool.py`.
...
- **RTT tracking:** `Stratum.send()` records a timestamp keyed by `(sock_id, msg_id)` after each send; `Pool.__on_client` calls `stats.on_receive()` on every incoming message that carries an `id`. A summary is logged on Ctrl+C.
- **Per-client state (workflow):** `StratumWorkflow` maintains a `dict[int, int]` (socket id → step index) protected by a `threading.Lock`. State is cleaned up via `on_disconnect()`.
